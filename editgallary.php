                            ...
                            <table class="table table-head-fixed text-nowrap">
                                ...
                                <tbody>
                                    ...
                                    if(isset($_REQUEST['delete'])){
                                        ...
                                        $did = $_POST['deleteid'];
                                        $res = mysqli_query($conn,"select * from gallary where id='".$did."'");
                                        $r = mysqli_fetch_assoc($res);
                                        rmdir("data/".$r['name']);
                                        unlink("data/".$r['image']);
                                        $deletequery = "Delete from gallary where id='".$did."'";
                                    ...
                                    if( mysqli_num_rows( $result )==0 ){
                                        ...
                                        while( $row = mysqli_fetch_assoc( $result ) ){
                                            echo "<tr><td >".$row['name']."</td><td colspan='1'><a href='data/".$row['image']."' target='_blank' >".$row['image']."</a><br/><br/> <img src='data/".$row['image']."' alt='Not loaded' width='200px' height='150px'/></td><td><a href='addDeleteGallery.php?name=".$row['name']."' >Add/Delete Images</a></td><td>
                                            <a href='editGalleryDetail.php?id=".$row['id']."'  class='btn btn-sm btn-primary w-25'><i class='fas fa-edit'></i></a>
                                            <button id='trigger' onclick='deletefunc(".$row['id'].")' class='btn btn-sm btn-danger w-25' ><i class='fas fa-trash'></i></button>
                                            <form method='post' action='editgallary.php'>
